...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // Hard filters after token recon — block launchpads, bots, 0% win rate pools, and rugcheck failures
    const filteredOut = [];
    const passing = allCandidates.filter(({ pool, ti, rc, memStats }) => {
      ...
      if (memStats && memStats.total_deploys >= 3 && memStats.win_rate === 0) {
        ...
        filteredOut.push({ name: pool.name, reason: `0% win rate over ${memStats.total_deploys} deploys` });
        return false;
      }
      return true;
    });
    ...
    const candidateBlocks = passing.map(({ pool, sw, n, ti, mem, rc, memStats }, i) => {
      const botPct = ti?.audit?.bot_holders_pct ?? "?";
